...
export default function AdminDocumentsPage() {
  ...
  const displayCustomer = (doc: Document) => {
    if (!doc.customer) return "Unassigned";
    if (typeof doc.customer === "object") {
      return `${doc.customer.first_name}${doc.customer.last_name ? ` ${doc.customer.last_name}` : ""} (${doc.customer.email})`;
    }
    return `ID: ${doc.customer}`;
  };
  ...
  const getFileTypeColor = (type: string) => {
    ...
    if (type.includes("json")) return "green";
    return "gray";
  };
  ...
  const filteredDocuments = useMemo(() => {
    ...
    return documents.filter((doc) => {
      ...
      switch (dateFilter) {
        ...
        case "week":
          return docDate.isAfter(now.subtract(1, "week"));
        case "month":
          return docDate.isAfter(now.subtract(1, "month"));
        case "year":
          return docDate.isAfter(now.subtract(1, "year"));
        default:
          return true;
      }
    });
              ...
              {isLoading ? (
                ...
                filteredDocuments.map((document) => (
                  <Table.Tr key={document._id}>
                    ...
                    <Table.Td>
                      {document.customer &&
                        typeof document.customer !== "string" ? (
                        <Box>
                          <Text size="sm" fw={500}>
